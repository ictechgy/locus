...
- **작성일**: 2026-09-05 (v0.3.0 세션 종료 시점) · **기준**: 워킹 트리 (미커밋 — 커밋 대기)
- **상태**: v0.1.0 → v0.3.0. 코드 리뷰 + P0(실전 검증) + P1(동적 스냅샷) + P2(문서·버전) 완료.
...
### P0' — 커밋 (사용자 승인 후 진행 가능)
워킹 트리에 리뷰 수정+v0.2+v0.3 전부 미커밋. 제안: 논리별 3커밋
(리뷰 수정 / P0 상수 해석·노이즈 / P1 스냅샷+문서) 또는 단일 커밋. 커밋 후 태그 `v0.3.0`.

### P2 잔여 — 공개 gate (사용자 결정 사항)
- 저장소명 `breadcrumb` 확정 여부: github.com/breadcrumb 존재함(별개 프로젝트).
  SPM 식별은 org/repo 조합이라 충돌 없음. 오염은 검색 관점( breadcrumb UI 패턴,
  Bootstrap breadcrumb 등) — 기획서 후보 `locus` 재검토 가능. **사용자 선택.**
- 리모트 생성·push → CI 실작동 확인 → 태그.
...
- 실전 검증 대상: element-x-ios (클론 위치 /tmp/breadcrumb-p0/ — 휘발성)
