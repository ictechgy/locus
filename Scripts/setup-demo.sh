...
# the README Quickstart's git-dependent step (`breadcrumb affected-tests` with a
...
# make it disappear. Zero-git alternative: `breadcrumb affected-tests
# --files Sources/ProfileView.swift`.
set -eu
cd "$(dirname "$0")/../Examples/DemoApp"

git init -q -b main
git config user.name "breadcrumb-demo"
git config user.email "demo@localhost"
git add -A
git commit -q -m "demo app baseline"

# One uncommitted change, so `breadcrumb affected-tests` has something to see.
